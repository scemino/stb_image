...
type
  STBIException* = object of CatchableError
...
proc loadFromMemory*(buffer: seq[byte]; x, y, channels_in_file: var int; desired_channels: int): seq[byte] =
  var
    ...
    castedBuffer = cast[ptr uint8](buffer[0].unsafeAddr)
...
proc stbi_is_hdr_from_memory(buffer: ptr uint8; len: cint): cint
...
proc isHDRFromMemory*(buffer: seq[byte]): bool =
  var castedBuffer = cast[ptr uint8](buffer[0].unsafeAddr)
...
proc stbi_zlib_decode_malloc_guesssize_headerflag(buffer: ptr uint8, len: cint, initial_size: cint,
  outlen: ptr cint, parse_header: cint): ptr uint8 {.importc: "stbi_zlib_decode_malloc_guesssize_headerflag", stbcall.}

proc stbi_zlib_decode_buffer(obuffer: ptr uint8, olen: cint, ibuffer: ptr uint8, ilen: cint): cint
  {.importc: "stbi_zlib_decode_buffer", stbcall.}

proc stbi_zlib_decode_noheader_buffer(obuffer: ptr uint8, olen: cint, ibuffer: ptr uint8, ilen: cint): cint
...
proc zlibDecodeMalloc*(buffer: openArray[byte], initial_size = 16384, parseheader = true): seq[byte] =
  ...
  let data = stbi_zlib_decode_malloc_guesssize_headerflag(cast[ptr uint8](unsafeAddr buffer[0]),
...
proc zlibDecodeBuffer*(input: openArray[byte], output: var openArray[byte], parseheader = true): Natural =
  let
    inputPtr = cast[ptr uint8](output[0].addr)
    outputPtr = cast[ptr uint8](input[0].unsafeAddr)
